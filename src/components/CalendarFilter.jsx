...
// Botón "Filtrar" con su menú desplegable (categoría y etiqueta).
export function CalendarFilterButton({ filter, onChange, rawEvents }) {
  ...
  return (
    <div className="calendar-filter" ref={rootRef}>
      <button
        ...
        aria-label={active ? `Filtro activo: ${filterLabel(filter)}` : 'Filtrar'}
        title={active ? `Filtro activo: ${filterLabel(filter)}` : 'Filtrar por categoría o etiqueta'}
      >
...
export function CalendarFilterBar({ filter, onChange }) {
  ...
  return (
    <div className="calendar-filter-bar" role="status">
      ...
      <span className="calendar-filter-bar-text">
        Solo se muestran las reuniones de <strong>{filterLabel(filter)}</strong>
      </span>
