...
// Se usa en la app y en la Edge Function de los recordatorios (supabase/functions/send-reminders).
// En la app las fechas son las del dispositivo; en el servidor, `toDate` las crea en la zona
// horaria del usuario (TZDate de @date-fns/tz), para que las series no se desplacen con los
// cambios de hora y cada día tenga la misma clave que en la app.
...
export function dateKey(date) {
  // Una fecha con zona (TZDate) se formatea en su zona.
...
export function expandEvent(event, rangeStart, rangeEnd, { toDate = localDate } = {}) {
  ...
  const untilDate = until ? endOfDay(toDate(until)) : endOfDay(addYears(baseStart, DEFAULT_HORIZON_YEARS))
